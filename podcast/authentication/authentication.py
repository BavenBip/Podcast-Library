...
def register():
    ...
    podcasts = services.get_podcast_data(repository.repo_instance)
    ...
    if form.validate_on_submit():
        ...
        try:
            services.add_user(form.user_name.data, form.password.data, repository.repo_instance)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        try:
            user = services.get_user(form.user_name.data, repository.repo_instance)

            # Authenticate user.
            services.authenticate_user(user['user_name'], form.password.data, repository.repo_instance)
